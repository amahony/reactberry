...
navTitle: Using Blocks
order: 3
---

# Using Blocks

After you have the provider wired up and are comfortable with `Box`, `Text`, `Button`, and `Field`, the next layer is `reactberry/blocks`.

Blocks are higher-level components and patterns built on top of the primitives.

## Importing blocks

```tsx
import { Container, Main, Heading, Divider } from "reactberry/blocks";
```
...
Use `reactberry/elements` for low-level building blocks:
...
Use `reactberry/blocks` when you want a precomposed pattern with more opinionated structure or behavior.

## Example page structure

```tsx
import { Box, Text, Button } from "reactberry/elements";
import { Container, Main, Heading, Divider } from "reactberry/blocks";
