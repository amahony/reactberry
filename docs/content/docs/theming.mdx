---
title: Theming
description: Use Reactberry's built-in themes and switch between light and dark mode with the design system provider.
navTitle: Theming
order: 2
---
...
`DesignSystemProvider` handles the design-system setup for you:
...
Reactberry primitives and blocks read values from the active theme, so common props like these should come from the design system instead of custom one-off CSS where possible:
...
Once theming is in place, move on to [Using Blocks](/docs/using-blocks) to build larger UI patterns on top of the primitives.
